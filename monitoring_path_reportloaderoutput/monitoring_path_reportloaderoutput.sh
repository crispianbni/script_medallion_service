...
###############################################################################
# Script Name  : monitoring_path_reportloaderoutput.sh
# Description  : Monitoring file baru pada path reportloaderoutput
...
STATE_FILE="${LOG_DIR}/.reportloaderoutput_last_state"
LOG_FILE="${LOG_DIR}/monitoring_path_reportloaderoutput.log"
SLEEP_INTERVAL=60
LAST_TODAY=""
...
check_and_log_new_files() {
    ...
    if [[ -n "${new_files}" ]]; then
        while read -r file; do
            log_message "reportloaderoutput" "${MONITOR_PATH}/${file}"
        done <<< "${new_files}"
    fi
