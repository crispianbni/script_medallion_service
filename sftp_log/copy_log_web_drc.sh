#!/bin/bash

###############################################################################
# Script Name  : copy_log_web_drc.sh
# Description  : Script untuk menyalin log medallionWeb.log dari local dan server WEB 2
# Author       : Crispian | 901146
# Division     : IT Application Services
# Last Update  : 22-02-2026
###############################################################################

REMOTE_HOST="192.168.230.41" # WEB 2
SOURCE_FILE="/opt/MedallionLog/web/medallionWeb.log"
LOCAL_DIR="/home/medallion/monitoring/automation/output_sftp_log/"
LOCAL_NAME1="medallionWeb_1.log"
LOCAL_NAME2="medallionWeb_2.log"

# Buat directory jika belum ada
mkdir -p "$LOCAL_DIR"

cp "$SOURCE_FILE" "$LOCAL_DIR/$LOCAL_NAME1"

sftp $REMOTE_HOST <<EOF
get $SOURCE_FILE $LOCAL_DIR/$LOCAL_NAME2
bye
EOF

# Validasi hasil
if [ $? -eq 0 ]; then
    echo "SUCCESS: server.log berhasil ditarik ke $LOCAL_DIR"
else
    echo "FAILED: Gagal menarik server.log"
fi